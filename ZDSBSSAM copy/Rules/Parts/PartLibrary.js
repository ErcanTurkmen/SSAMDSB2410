...
export default class {
    ...
    static partCreateUpdateValidation(pageClientAPI) {
        ...
        //dict.StorageLocationLstPkr.clearValidation(); DSB customisation to remove storage location mandatory validation and set as it was in 2110
        ...
        //DSB customisation
        dict.RecipientSim.clearValidation();
        dict.UnloadingPointSim.clearValidation();
        ...
        validations.push(libThis.validatePlantNotBlank(pageClientAPI, dict));
        //DSB customisation to remove storage location mandatory validation and set as it was in 2110
        //validations.push(libThis.validateStorageLocationNotBlank(pageClientAPI, dict));
    ...
    static partIssueCreateUpdateValidation(pageClientAPI) {
        ...
        const dict = libCom.getControlDictionaryFromPage(pageClientAPI);
        let pageName = libCom.getPageName(pageClientAPI.getPageProxy());
    ...
    static partReturnValidation(pageClientAPI) {
        ...
        //DSb customisation - in std return validation, added validation for serial no for adhoc returnb. cant use std isSerial as we dont have data like wo components
        // and for material
        let pageName = libCom.getPageName(pageClientAPI.getPageProxy());
        if (pageName === 'ZPartAdhocReturnCreateUpdate') {
            ...
            if (libCom.isDefined(dict.SerialNumLstPkr) && dict.SerialNumLstPkr.visible) {
                ...
                //since we are on adhoc return, there is no issued qty
    ...
    */
    ...
    /**
        * Sets line item values for create/update service before writing OData record for adhoc part issue
        */
    static zPartAdhocMovementLineItemCreateUpdateSetODataValue(pageClientAPI, key, movementType) {

        switch (key) {
            case 'OrderNumber':
                //return pageClientAPI.evaluateTargetPath('#Property:OrderId');
                return pageClientAPI.binding.OrderId;
            case 'OperationNo':
                //return pageClientAPI.evaluateTargetPath('#Property:OrderId');
                return pageClientAPI.binding.OperationNo;
            case 'MovementType':
                return libCom.getAppParam(pageClientAPI, 'WORKORDER', movementType);
            case 'ReservationNumber':
                //return pageClientAPI.evaluateTargetPath('#Property:RequirementNumber');
            ...
            case 'Material':
                //return pageClientAPI.evaluateTargetPath('#Property:MaterialNum');
                //return libCom.getListPickerValue(libCom.getFieldValue(pageClientAPI, 'MaterialLstPkr', '', null, true));
                return SplitReadLink(libCom.getListPickerValue(libCom.getFieldValue(pageClientAPI, 'MaterialLstPkr', '', null, true))).MaterialNum;
            case 'Plant':
                //return pageClientAPI.evaluateTargetPath('#Property:Plant');
                return libCom.getListPickerValue(libCom.getFieldValue(pageClientAPI, 'PlantLstPkr', '', null, true));
            case 'StorageLocation':
                return libCom.getListPickerValue(libCom.getFieldValue(pageClientAPI, 'StorageLocationLstPkr', '', null, true));
            case 'ValuationType':
                return libCom.getListPickerValue(libCom.getFieldValue(pageClientAPI, 'ValuationTypePicker', '', null, true))
            case 'Quantity':
                return libCom.getFieldValue(pageClientAPI, 'QuantitySim', '', null, true);
            case 'EntryUOM':
                return libCom.getListPickerValue(libCom.getFieldValue(pageClientAPI, 'MaterialUOMLstPkr', '', null, true));
            default:
                return '';
        }
    }
    ...
    /**
      ...
      * @param {*} context
      */
    static zPartAdhocIssueCreateLineItemSuccess(context) {
        context.getClientData().isIssue = true;
