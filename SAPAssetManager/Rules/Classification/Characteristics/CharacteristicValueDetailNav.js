...
export default function CharacteristicValueDetailNav(context) {
    ...
    return context.read('/SAPAssetManager/Services/AssetManager.service', entitySet, [], '$filter=ObjectKey eq \'' + binding.ObjectKey + '\' and CharId eq \'' + context.getPageProxy().getActionBinding().Characteristic.CharId + '\'' + '&$expand=Characteristic/ClassCharacteristics/Characteristic/CharacteristicValues,CharValCode_Nav&$orderby=CharId').then(function(results) {
        if (results.getItem(0)) {
            let actionBinding = results.getItem(0);
            actionBinding.InternClassNum = binding.InternClassNum;
