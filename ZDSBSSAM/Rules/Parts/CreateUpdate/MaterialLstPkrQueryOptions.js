...
export default function MaterialLstPkrQueryOptions(context) {
    try {
        ...
        if (isMultipleTechnician && pageName !== 'PartCreateUpdatePage'  && pageName !== 'PartIssueCreateUpdatePage' && pageName !== 'ZPartAdhocIssueCreateUpdate' && pageName !== 'ZPartAdhocReturnCreateUpdate') { // we need that part only if we are on VehicleIssueOrReceiptCreatePage or VehiclePartCreate
        ...
        if (context.searchString) {
            if (onlineSwitchValue) {
                let searchString = context.searchString;
                let filters = [
                    //DSB customisation to enable search by material num and not desc
                    //`substringof('${searchString.toLowerCase()}', MaterialDesc)`,
                    `substringof('${searchString.toLowerCase()}', MaterialNum)`,
                ];
