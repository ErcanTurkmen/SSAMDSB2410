...
export default function ZPRTEquipmentCreateUpdateValidation(pageClientAPI) {
    ...
    return pageClientAPI.read('/SAPAssetManager/Services/AssetManager.service', 'MyEquipClassCharValues', [], "$filter=EquipId eq '" + equipid + "' and CharId eq '" + charId + "' &$orderby=CharId,EquipId,CharValDesc asc,CharValFrom asc").then(function(results) {
        //for (var i = 0; i < results.length; i++) {
            if(results.length>0)
            {
                    ...
                    var charVal = results.getItem(lastUpdatedValueIndex).CharValFrom;
