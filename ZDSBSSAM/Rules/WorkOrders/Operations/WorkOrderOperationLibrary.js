...
export class OperationEventLibrary {
    ...
    static setDefaultValues(pageProxy) {
        ...
        parentWorkOrderPromise.then(parentWorkOrder => {
            ...
            //DSB customisation to set ctrl key to SM01 always
            //controlKeyCtrl.setValue(parentWorkOrder.OrderType);
            controlKeyCtrl.setValue(`${ZOperationControlKey}`);
...
export class OperationConstants {
    //DSB customisation to order by ZPriorityOpr asc
    ...
    static OperationListQueryOptions(context) {
        const expandFields = ['WOObjectList_Nav', 'Tools', 'OperationMobileStatus_Nav', 'OperationMobileStatus_Nav/OverallStatusCfg_Nav', 'OperationLongText', 'WOHeader', 'WOHeader/OrderMobileStatus_Nav', 'WOHeader/UserTimeEntry_Nav', 'UserTimeEntry_Nav', 'WOHeader/WOPriority', 'Employee_Nav', 'WOOprDocuments_Nav/Document, NotifHeader_Nav'];
        if (userFeaturesLib.isFeatureEnabled(context, context.getGlobalDefinition('/SAPAssetManager/Globals/Features/QM.global').getValue())) {
