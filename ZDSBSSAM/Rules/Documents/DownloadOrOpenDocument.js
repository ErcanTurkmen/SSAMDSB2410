...
import setFileInfo from '../../../SAPAssetManager/Rules/Documents/DocumentEditorSetFileInfo';
...
import { isSignatureDeleteEnabled } from '../../../SAPAssetManager/Rules/UserAuthorizations/WorkOrders/EnableSignatureDiscard';
...
export default function DownloadOrOpenDocument(sectionedTableProxy, documentPropertyName) {
    ...
    if (documentObject.ObjectType === 'URL') {
        ...
        return sectionedTableProxy.isMediaLocal(serviceName, entitySet, readLink).then((isMediaLocal) => {
            // DSB customisation. No exchange for document download. So checking every hours 8 to download the latest document
            ...
            let lastModified = documentFileObject.lastModified;
            let lastModiFileDate = new Date(lastModified);

            const today = currentDate;
            const endDate = lastModiFileDate;
            let days = parseInt(Math.abs(endDate - today) / (1000 * 60 * 60 * 24));
            ...
            if (isMediaLocal && (hours <=8 && days ==0 )) {
                let documentPath = DocumentPath(sectionedTableProxy, documentObject);
